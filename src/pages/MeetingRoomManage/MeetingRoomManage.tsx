...
import './meeting-room-manage.css';
import { ColumnsType } from "antd/es/table";
import { useForm } from "antd/es/form/Form";
import { deleteMeetingRoom, getMeetingRoomList } from "../../api/meeting-room";
...
export function MeetingRoomManage() {
      ...
      render: (_, record) => (
        ...
        >
          ...
          <br/>
            ...
            <a onClick={() => handleUpdate(record.id)}>更新</a>
        </Popconfirm>
      )
    }
  ], [handleDelete]);
  ...
  const searchMeetingRoom = useCallback(async (values: SearchMeetingRoom) => {
    const res = await getMeetingRoomList(values.name, values.capacity, values.equipment, pageNo, pageSize);
    const { data } = res.data;
  ...
  }, [form, num, searchMeetingRoom]);


  return <div id="meeting-room-manage-container">
    ...
    <CreateMeetingRoomModal isOpen={isCreateModalOpen} handleClose={handleCreateModalClose}></CreateMeetingRoomModal>
    <UpdateMeetingRoomModal id={updateId!} isOpen={isUpdateModalOpen} handleClose={handleUpdateModalClose}></UpdateMeetingRoomModal>
  </div>
}
